...
const Testimonials = () => {
  ...
  useEffect(() => {
    const socket = io("https://birur-trading-backend.vercel.app");

    const fetchProducts = () => {
      fetch("https://birur-trading-backend.vercel.app/api/cereals")
        .then((res) => res.json())
    ...
    socket.on("cerealsUpdated", () => {
      fetchProducts(); // refresh when backend changes
    });
  ...
  return (
    <div id="testimonial" className="py-20">
      <div className="w-full text-center">
        <p className="text-4xl font-bold font-cursive text-amber-900">
          Samples
        </p>
      </div>
      <div className="mx-auto max-w-7xl">
        <div className="w-full max-w-6xl px-6 mx-auto">
          <Slider {...settings}>
            {products.map((data) => (
              <div key={data.id} className="flex justify-center px-2 py-6">
                <div className="w-full max-w-[410px] flex flex-col gap-4 p-4 shadow-lg rounded-xl bg-amber-900/10">
                  <div
                    className="cursor-pointer"
                    onClick={() =>
                      setSelectedImage(
                        `https://birur-trading-backend.vercel.app${data.media}?t=${Date.now()}`,
                      )
                    }
                  >
                    {data.media?.endsWith(".mp4") ? (
                      ...
                      >
                        <source
                          src={`https://birur-trading-backend.vercel.app${data.media}`}
                        />
                      </video>
                    ) : (
                      <img
