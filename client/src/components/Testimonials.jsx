...
const Testimonials = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = () => {
      fetch("http://localhost:5000/api/cereals")
        .then((res) => res.json())
              ...
              <div key={data.id} className="flex justify-center px-2 py-6">
                <div className="w-full max-w-[410px] flex flex-col gap-4 p-4 shadow-lg rounded-xl bg-amber-900/10">
                  <div
                    ...
                    onClick={() =>
                      setSelectedImage(
                        `http://localhost:5000${data.media}?t=${Date.now()}`,
                      )
                    }
                  >
                    {data.media?.endsWith(".mp4") ? (
                      <video
                        ...
                        controls
                      >
                        <source src={`http://localhost:5000${data.media}`} />
                      </video>
                    ) : (
                      <img
                        src={`http://localhost:5000${data.media}`}
                        alt={data.name}
                        className="w-full h-[320px] object-cover rounded-md"
                      />
                    )}
